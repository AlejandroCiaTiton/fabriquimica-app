...
import { useAvanzarEstadoOC, useActualizarDatosOC, useActualizarEstadoPago } from '../../hooks/useOrdenes'
...
import { fmtUSD as _fmtUSD } from '../../utils/calc'

export const PIPELINE = ['recibida', 'en-preparacion', 'listo-entrega', 'entregada']
export const LABELS   = { recibida: 'Recibida', 'en-preparacion': 'En preparación', 'listo-entrega': 'Listo p/entrega', entregada: 'Entregada' }

export const fmtUSD = _fmtUSD

export function PipelineBar({ estado }) {
...
function FilaAccionCorrectiva({ oc, item, rec }) {
  ...
  async function handleSolicitarNC() {
    ...
    setNcEnviada(true); setShowNC(false); setNcMonto(''); setNcMotivo('')
  }
  ...
  return (
    <div className="p-3 border-t border-gray-100">
      ...
      {rec.comentario && <p className="text-xs text-gray-500 italic mb-1.5">Cliente: "{rec.comentario}"</p>}
      {rec.foto_url && (
      ...
      <div className="flex gap-1 mb-3 flex-wrap">
        {GESTION_OPTS.map(opt => (
          <button key={opt.key} onClick={() => { setEstado(opt.key); save(opt.key) }} disabled={guardar.isPending}
      ...
      {ncEnviada ? (
        <p className="text-xs text-green-700 bg-green-50 rounded-lg px-3 py-2 font-medium">✓ Solicitud de NC enviada a Finanzas</p>
      ) : !showNC ? (
        <button onClick={() => { setNcMotivo(rec.comentario ? `Reclamo de calidad: ${rec.comentario}` : ''); setShowNC(true) }}
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-3 space-y-2">
          ...
          <div className="flex gap-2">
            <div className="w-28">
              ...
              <input type="number" min="0" step="0.01" value={ncMonto} onChange={e => setNcMonto(e.target.value)}
          ...
          <div className="flex gap-2">
            <button onClick={handleSolicitarNC} disabled={!ncMonto || !ncMotivo.trim() || solicitarNC.isPending}
            ...
            <button onClick={() => setShowNC(false)} className="text-xs text-gray-500 hover:text-gray-700 px-2 py-1.5">Cancelar</button>
          </div>
        </div>
      )}
    </div>
...
function PanelRecepcion({ oc }) {
  ...
  if (oc.recepcion_confirmada) {
    const itemsProb = items.filter(it => { const r = getRec(it.id); return r && (r.estado === 'prob' || r.estado === 'rech') })
    return (
      <div className="mt-3 border border-gray-100 rounded-lg overflow-hidden">
        <div className="bg-gray-50 px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide">Recepción confirmada</div>
        {itemsProb.map(item => <FilaAccionCorrectiva key={item.id} oc={oc} item={item} rec={getRec(item.id)} />)}
        {itemsProb.length === 0 && (
          <div className="px-3 py-3 text-xs text-[#28a745] font-medium">✓ Recepción conforme — sin problemas reportados</div>
        )}
      </div>
    )
  }
  ...
  return (
    <div className="mt-3 border border-blue-100 rounded-lg overflow-hidden">
        ...
        return (
          <div key={item.id} className="p-3 border-t border-gray-100">
            <p className="text-sm font-medium text-gray-900 mb-2">
              {item.productos?.nombre}<span className="text-xs text-gray-400 ml-2">{item.cantidad} kg/lt</span>
            </p>
      ...
      <div className="px-3 py-3 border-t border-gray-100 bg-gray-50">
        {!todosMarcados && <p className="text-xs text-gray-400 mb-2">Faltan confirmar {items.filter(it => !getRec(it.id)?.estado).length} producto(s)</p>}
        <button onClick={() => confirmar.mutate({ ocId: oc.id })} disabled={!todosMarcados || confirmar.isPending}
  ...
  return (
    <div className="mt-3 p-3 bg-gray-50 border border-gray-100 rounded-lg">
      ...
      <div className="flex gap-2 flex-wrap">
        {Object.entries(PAGO_CONFIG).map(([opt, cfg]) => (
          <button key={opt}
            onClick={() => opt !== actual && actualizar.mutate({ ocId: oc.id, estadoPago: opt })}
            disabled={actualizar.isPending}
            className={`text-xs font-semibold px-3 py-1.5 rounded-lg border transition-all disabled:opacity-50 ${
              actual === opt ? cfg.cls + ' font-bold' : 'border-gray-200 text-gray-400 hover:border-gray-300 hover:text-gray-600 bg-white'
            }`}>
            {cfg.label}
          </button>
        ))}
...
// ── Card principal exportada ──────────────────────────────────────────────────
export default function OCCard({ oc, isNew, facturaPdfUrl, showVendedor = false }) {
      ...
      <div className="flex items-start justify-between gap-3 mb-1">
        <div>
          ...
          {showVendedor && vendedor && (
            <p className="text-xs text-gray-400 mt-0.5">Vendedor: <span className="font-medium text-gray-600">{vendedor}</span></p>
          )}

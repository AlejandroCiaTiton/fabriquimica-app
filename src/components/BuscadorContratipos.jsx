...
export default function BuscadorContratipos({
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 py-2 text-sm text-gray-400">
        <div className="w-3.5 h-3.5 border-2 border-gray-300 border-t-[#1b4332] rounded-full animate-spin"/>
  ...
  const SELECT = 'w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#1b4332] bg-white'
  ...
  return (
    <div className="space-y-2">
      ...
      {/* 3. Equivalente FQ */}
      {contratipo && (
        <div className="border border-[#1b4332]/20 rounded-lg p-3 bg-blue-50/40 space-y-2">
          <div className="flex items-start gap-2">
            <svg className="w-3.5 h-3.5 text-[#1b4332] flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3"/>
            </svg>
            <div className="flex-1 min-w-0">
              {productoFQ ? (
                <>
                  <p className="text-sm font-semibold text-[#1b4332] leading-snug">{productoFQ.nombre}</p>
          ...
          {productoFQ && !idsExcluir.has(productoFQ.id) && (
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 bg-[#1b4332] hover:bg-[#152e24] text-white text-xs font-medium py-1.5 rounded-lg transition-colors"
            >

...
function NumInputUSD({ value, onSave, step = 1000 }) {
  ...
  if (editando) {
    return (
      <input
        ...
        className="w-full text-center text-sm border border-[#1b4332] rounded px-1 py-1.5 focus:outline-none"
      />
    )
  }
  ...
  return (
    <div>
      ...
      >
        {value > 0
          ? <span className="text-sm font-bold text-[#1b4332]">${Number(value).toLocaleString('es-AR')}</span>
...
function NumInputAnual({ value, onSave }) {
    ...
    return (
      <div className="flex items-center gap-1">
        <input
          ...
          onBlur={guardar}
          onKeyDown={e => { if (e.key === 'Enter') guardar(); if (e.key === 'Escape') { setVal(value ?? 0); setEditando(false) } }}
          className="w-36 text-right text-sm border border-[#1b4332] rounded-lg px-2 py-1.5 focus:outline-none"
        />
      </div>
    )
  }
  ...
  return (
    <button
      onClick={() => { setVal(value ?? 0); setEditando(true) }}
      className="flex items-center gap-1.5 text-sm font-semibold text-gray-800 hover:text-[#1b4332] transition-colors group"
    >
      {value > 0
        ? `USD ${Number(value).toLocaleString('es-AR')}`
        : <span className="text-gray-300 font-normal">Clic para ingresar</span>
      }
      <svg className="w-3.5 h-3.5 text-gray-300 group-hover:text-[#1b4332] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  return (
    <div className="p-6 max-w-5xl">
      ...
      <div className="flex gap-1 bg-white border border-gray-200 rounded-lg p-1 w-fit mb-6">
        {AÑOS.map(a => (
          <button key={a} onClick={() => setAño(a)}
            className={`px-4 py-1.5 rounded text-sm font-medium transition-colors ${año === a ? 'bg-[#1b4332] text-white' : 'text-gray-500 hover:text-gray-800'}`}>
            {a}
          </button>
        ))}
      </div>
      ...
      {(totalAnual > 0 || sumaMensual > 0) && (
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-[10px] shadow-card px-5 py-4">
            ...
            <p className="text-2xl font-bold text-[#1b4332]">USD {totalAnual.toLocaleString('es-AR')}</p>
      ...
      <div className="bg-white rounded-[10px] shadow-card overflow-hidden mb-6">
        ...
        {loadVend || loadPlan
          ? <div className="flex justify-center py-10"><div className="w-6 h-6 border-4 border-[#1b4332] border-t-transparent rounded-full animate-spin"/></div>
          : (
            <div>
              {planes.length > 0 && (
                          ...
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-3">
                              <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden max-w-[180px]">
                                <div className="h-full bg-[#1b4332] rounded-full transition-all" style={{ width: `${Math.min(pct, 100)}%` }}/>
                              </div>
                  ...
                  {totalAnual > 0 && (
                    <tfoot>
                      <tr className="border-t-2 border-gray-200 bg-gray-50">
                        ...
                        <td className="px-5 py-3 text-right font-bold text-[#1b4332]">USD {totalAnual.toLocaleString('es-AR')}</td>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {vendedoresSinPlan.map(v => (
                      <button key={v.id}
                        ...
                        className="flex items-center gap-2 text-sm border border-dashed border-gray-300 text-gray-500 hover:border-[#1b4332] hover:text-[#1b4332] px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50"
                      >
                ...
                return (
                  <div key={i} className="flex flex-col items-center">
                    <div className="flex flex-col justify-end h-14 w-full">
                      <div className="w-full bg-[#1b4332]/15 rounded-t-sm transition-all" style={{ height: `${h}px`, minHeight: v > 0 ? '2px' : 0 }}/>
                    </div>
                  </div>
                )
              })}

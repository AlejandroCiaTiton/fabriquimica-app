  ...
  if (editing) {
    return (
      <input type="number" step="0.01" value={val} autoFocus
        ...
        className="w-24 text-right text-sm border border-[#004a99] rounded px-2 py-0.5 focus:outline-none"
      />
    )
  }
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          {!filas && !resultado && (
            <div
              ...
              className={`border-2 border-dashed rounded-xl p-10 text-center cursor-pointer transition-colors ${
                drag ? 'border-[#004a99] bg-blue-50' : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
              }`}
            >
          ...
          {resultado && (
            <div className={`rounded-lg p-5 text-center ${resultado.fail === 0 ? 'bg-green-50 border border-green-200' : 'bg-yellow-50 border border-yellow-200'}`}>
              ...
              <button onClick={onClose} className="mt-4 px-5 py-2 bg-[#004a99] text-white text-sm font-medium rounded-lg hover:bg-[#003a7a]">
                Cerrar
              </button>
            </div>
          )}
                      ...
                      <tr key={i} className={r.encontrado ? 'hover:bg-gray-50' : 'bg-red-50 opacity-60'}>
                        ...
                        {['lista1_may','lista4_std','lista3_min'].map(campo => (
                          <td key={campo} className="px-3 py-1.5 text-right">
                            {r[campo] !== undefined
                              ? <span className="font-medium text-[#004a99]">{r[campo] != null ? Number(r[campo]).toFixed(2) : '—'}</span>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-[#004a99] text-white rounded-lg hover:bg-[#003a7a]"
          >

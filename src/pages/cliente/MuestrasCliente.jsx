...
export default function MuestrasCliente() {
  ...
  const { data: muestras    = [], isLoading: loadM } = useMuestrasCliente()
  const { data: solicitudes = [], isLoading: loadS } = useSolicitudesCliente()
        ...
        <>
          ...
          {!loadS && solicitudes.length > 0 && (
            <div className="space-y-3">
              {solicitudes.map(s => {
                const cfg = SOLIC_CFG[s.estado] ?? { label: s.estado, cls: 'bg-gray-100 text-gray-500', dot: 'bg-gray-300' }
                return (
                  <div key={s.id} className="bg-white rounded-[10px] shadow-card p-4">
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <div>
                        {s.productos
                          ? <p className="text-sm font-semibold text-gray-900">{s.productos.nombre}</p>
                          : <p className="text-sm font-semibold text-gray-400 italic">Sin producto específico</p>
                        }
                        <p className="text-xs text-gray-400">{fmtFecha(s.creado_en)}</p>
                      </div>
                      <span className={`flex-shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-full ${cfg.cls}`}>{cfg.label}</span>
                    </div>
                    <p className="text-xs text-gray-600 italic mb-2">"{s.uso_previsto}"</p>

                    {s.respuesta_vendedor && (
                      <div className={`rounded-lg px-3 py-2 text-xs ${s.estado === 'rechazada' ? 'bg-red-50 border border-red-100 text-red-700' : 'bg-blue-50 border border-blue-100 text-blue-700'}`}>
                        <span className="font-semibold">Respuesta del vendedor:</span> {s.respuesta_vendedor}
                      </div>
                    )}

                    {s.estado === 'alternativa_sugerida' && s.producto_alternativo && (
                      <div className="mt-2 bg-blue-50 border border-blue-100 rounded-lg px-3 py-2 text-xs text-blue-800">
                        <span className="font-semibold">Producto alternativo sugerido:</span> {s.producto_alternativo.nombre}
                      </div>
                    )}
                  </div>
                )
